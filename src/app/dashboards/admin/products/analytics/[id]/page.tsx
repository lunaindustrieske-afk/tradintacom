...
export default function AdminProductAnalyticsPage() {
  ...
  const productDocRef = useMemoFirebase(() => {
    if (!manufacturerId || !firestore || !productId) return null;
    return doc(firestore, 'manufacturers', manufacturerId, 'products', productId);
  }, [firestore, manufacturerId, productId]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push(`/dashboards/admin/shops/${manufacturerId}`);
    } catch (error: any) {
      toast({
        title: 'Error',
  ...
  if (!product) {
    return (
        <div>
            ...
            <p className="text-muted-foreground">The requested product could not be loaded. Check the URL and permissions.</p>
             <Button variant="link" asChild><Link href="/dashboards/admin">Go Back to Admin Dashboard</Link></Button>
        </div>
    )
  }
        ...
        <BreadcrumbList>
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link href="/dashboards/admin">Admin</Link>
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
           <BreadcrumbItem>
             <BreadcrumbLink asChild>
               <Link href={`/dashboards/admin/shops/${manufacturerId}`}>Shop Management</Link>
             </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Product Analytics</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" className="h-7 w-7" asChild>
          <Link href={`/dashboards/admin/shops/${manufacturerId}`}>
        ...
        <div className="hidden items-center gap-2 md:ml-auto md:flex">
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive" size="sm">
                <Trash2 className="mr-2" /> Delete Product
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete this product from the seller's catalog.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <Button asChild size="sm">
            <Link href={`/dashboards/seller-centre/products/edit/${productId}`} target="_blank">
              <Edit className="mr-2" /> Edit Product (Seller View)
            </Link>
          </Button>
        </div>
      </div>
